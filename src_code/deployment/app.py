...
with st.form("Sales Estimation"):
    # Product Inputs with min/max constraints
    ...
    with col2:
        prefix = CATEGORY_PREFIX_MAP.get(product_type, "FD")
        st.write("Product ID")
        sub_col1, sub_col2 = st.columns([1, 3])
        with sub_col1:
            st.text_input("Prefix", value=prefix, disabled=True, label_visibility="collapsed")
        with sub_col2:
